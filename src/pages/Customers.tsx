      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs lg:text-sm font-medium">Manage your CRM and accounts receivable.</p>
          </div>
          <button 
            onClick={() => business ? setIsAddModalOpen(true) : navigate('/onboarding')}
            className="flex-1 md:flex-none px-6 py-3 bg-blue-600 text-white rounded-2xl text-xs font-bold uppercase tracking-widest active:scale-95 transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-100"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
           <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3">
              ...
              <div>
                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Total Clients</p>
                 <p className="text-base font-bold text-slate-900 tracking-tight">{customers.length}</p>
              </div>
           </div>
           <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3">
              ...
              <div>
                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Total Dues</p>
                 <p className="text-base font-bold text-red-500 tracking-tight">{formatBDT(customers.reduce((acc, c) => acc + c.total_due_cents, 0))}</p>
              </div>
           </div>
           <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3">
              ...
              <div>
                 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Collectors</p>
                 <p className="text-base font-bold text-emerald-600 tracking-tight">{customers.filter(c => c.total_due_cents > 0).length}</p>
              </div>
           </div>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            className="w-full bg-white border border-slate-100 h-10 pl-10 pr-4 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all shadow-sm text-xs font-medium"
...
function CustomerCard({ customer, onClick, onEdit, onDelete }: { customer: Customer, onClick: () => void, onEdit: () => void, onDelete: () => void }) {
  ...
  return (
    <motion.div 
      layout
      onClick={onClick}
      className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-100 transition-all group flex flex-col cursor-pointer active:scale-95"
    >
       <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
             ...
             <div className="text-left">
                <h3 className="text-sm lg:text-base font-semibold text-slate-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{customer.name}</h3>
                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{customer.shop_name || 'Individual'}</p>
             </div>
          </div>
       ...
       <div className="space-y-2 mb-4 text-left">
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500">
             <Phone className="w-3 h-3 text-slate-300" /> {customer.phone || 'N/A'}
          </div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-500 truncate">
             <MapPin className="w-3 h-3 text-slate-300" /> {customer.address || 'No address provided'}
          </div>
       </div>

       <div className="mt-auto pt-3 border-t border-slate-50 flex items-center justify-between">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest">Balance Due</span>
          <p className={`text-base font-bold tracking-tight ${isDue ? 'text-red-500' : 'text-emerald-500'}`}>
